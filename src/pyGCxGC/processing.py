...
def mask_chromatogram(
    chromatogram: pd.DataFrame | np.ndarray, mask_path: str | Path
    ...
    if mask.shape != chromatogram.shape:
        raise ValueError(
            f"Mask shape {mask.shape} does not match chromatogram shape {chromatogram.shape}."
        )
    if np.max(mask) == 255:
...
def integrate_2D(Chrom_2D: pd.DataFrame | np.ndarray) -> float:
    """
...
def integrate_masks(
    chromatogram_2D: pd.DataFrame | np.ndarray,
    masks: list | Path | str,
    mask_names: list | str = "infer",
) -> dict:
    """
    ...
    if isinstance(masks, (str, Path)):
        ...
        raise TypeError(
            "masks must be a list of paths or a path to a directory containing the masks"
        )

    # Infer mask names if needed
    if mask_names == "infer":
        mask_names = [Path(mask).stem for mask in mask_list]
