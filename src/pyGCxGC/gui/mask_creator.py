...
class MaskCreatorGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.original_chromatogram = (
            None  # Will hold the original unshifted chromatogram
        )
    ...
    def _create_left_panel(self, parent):
        ...
        load_btn = ttk.Button(
            chrom_frame, text="Load Chromatogram File", command=self._load_chromatogram
        )
        ...
        shift_entry = ttk.Entry(shift_frame, textvariable=self.shift_value, width=8)
        shift_entry.pack(fill=tk.X, pady=2)
        create_tooltip(
            shift_entry,
            "Enter shift value (integer) to adjust phase of the chromatogram",
        )
        ...
        create_tooltip(
            transform_raw_btn, "Display raw chromatogram data without transformation"
        )
        ...
        create_tooltip(
            transform_sqrt_btn, "Display square root of chromatogram data (default)"
        )
        ...
        create_tooltip(
            transform_thirdroot_btn, "Display third root of chromatogram data"
        )

        # Apply button spanning the full width at the bottom
        apply_shift_btn = ttk.Button(
            plotting_frame, text="Apply", command=self._apply_shift
        )
        apply_shift_btn.pack(fill=tk.X, pady=(10, 5))
        create_tooltip(
            apply_shift_btn,
            "Apply the shift and transform to the original chromatogram and update the display",
        )
        ...
        create_tooltip(
            polygon_btn,
            "Draw polygon selections with straight line segments on the chromatogram",
        )

        # Clear button
        clear_btn = ttk.Button(
            draw_frame, text="Clear Current Selection", command=self._clear_selection
        )
        ...
        name_entry = ttk.Entry(mask_frame, textvariable=self.mask_name)
        name_entry.pack(fill=tk.X, pady=5)
        create_tooltip(name_entry, "Enter a name for your mask (used when saving)")

        # Add current selection to mask
        add_btn = ttk.Button(
            mask_frame, text="Add Selection to Mask", command=self._add_to_mask
        )
        ...
        new_btn = ttk.Button(
            mask_frame, text="Create New Mask", command=self._create_new_mask
        )
        ...
        save_btn = ttk.Button(mask_frame, text="Save Mask", command=self._save_mask)
        save_btn.pack(fill=tk.X, pady=5)
        create_tooltip(save_btn, "Save the current mask as a TIFF file")

        load_mask_btn = ttk.Button(
            mask_frame, text="Load Mask", command=self._load_mask
        )
    ...
    def _load_chromatogram(self):
        ...
        try:
            ...
            ttk.Label(modulation_dialog, text="Enter modulation time in seconds:").pack(
                pady=10
            )

            modulation_time_var = tk.DoubleVar(value=20.0)
            modulation_entry = ttk.Entry(
                modulation_dialog, textvariable=modulation_time_var
            )
            ...
            self.status_var.set(f"Loading chromatogram from {Path(file_path).name}...")
            self.chromatogram = gcgc.parse_2D_chromatogram(
                file_path, modulation_time=modulation_time, sampling_interval="infer"
            )
    ...
    def _display_chromatogram(self):
        ...
        if self.mask is not None:
            ...
            self.status_var.set(
                f"Display updated. Mask covers {masked_pixels} pixels ({percent_masked:.2f}% of total)"
            )
        else:
    ...
    def _update_selector(self):
        ...
        if self.drawing_mode.get() == "rectangle":
            self.selector = RectangleSelector(
                ...
                interactive=True,
                props=dict(
                    color="white", alpha=0.2, fill=True
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f"Failed to create polygon selector: {str(e)}"
                )
                traceback.print_exc()
    ...
    def _on_rectangle_select(self, eclick, erelease):
        ...
        self.status_var.set(
            f"Rectangle selected: RT1 range = {min(x1, x2):.2f}-{max(x1, x2):.2f} min, RT2 range = {min(y1, y2):.2f}-{max(y1, y2):.2f} s"
        )
    ...
    def _on_polygon_select(self, xdata, ydata):
        ...
        vertices = list(zip(xdata, ydata, strict=True))
    ...
    def _on_polygon_callback(self, vertices):
        ...
        # If it's an array of shape (n,2), convert to list of tuples
            ...
            hasattr(vertices, "shape")
            and len(vertices.shape) == 2
        ...
        # Otherwise try to interpret vertices as x,y arrays
            ...
            try:
                ...
                self.selected_points = {
                    "type": "polygon",
                    "vertices": list(map(tuple, vertex_points)),
                }
            except Exception as e:
    ...
    def _add_to_mask(self):
        ...
        if self.chromatogram is None or self.mask is None:
            messagebox.showwarning(
                "Warning", "Please load a chromatogram and create a mask first"
            )
            return

        # For polygon selection, get vertices from the active polygon selector
        if self.drawing_mode.get() == "polygon" and hasattr(
            self, "get_polygon_vertices"
        ):
        ...
        try:
            ...
            data_to_array_x = lambda x: int(
                (x - self.chromatogram.limits[0])
            ...
            data_to_array_y = lambda y: int(
                (self.chromatogram.limits[3] - y)
            ...
            if (
                isinstance(self.selected_points, dict)
                and "type" in self.selected_points
            ):
                ...
                if selection_type == "rectangle":
                    try:
                        ...
                        x1_idx = max(
                            0, min(data_to_array_x(x1), self.mask.shape[1] - 1)
                        )
                        y1_idx = max(
                            0, min(data_to_array_y(y1), self.mask.shape[0] - 1)
                        )
                        x2_idx = max(
                            0, min(data_to_array_x(x2), self.mask.shape[1] - 1)
                        )
                        y2_idx = max(
                            0, min(data_to_array_y(y2), self.mask.shape[0] - 1)
                        )
                        ...
                        self.status_var.set(
                            f"Added rectangle selection to mask ({x2_idx - x1_idx + 1}x{y2_idx - y1_idx + 1} pixels)"
                        )
                    except Exception as e:
                        messagebox.showerror(
                            "Error", f"Failed to process rectangle selection: {str(e)}"
                        )
                        traceback.print_exc()
                    ...
                    try:
                        ...
                        array_to_data_x = lambda x: self.chromatogram.limits[0] + (
                            x / self.mask.shape[1]
                        ) * (self.chromatogram.limits[1] - self.chromatogram.limits[0])

                        # Invert the y coordinate to match the display orientation
                        array_to_data_y = lambda y: self.chromatogram.limits[3] - (
                            y / self.mask.shape[0]
                        ) * (self.chromatogram.limits[3] - self.chromatogram.limits[2])

                        # Transform points to data coordinates
                        points_data = np.vstack(
                            (
                                array_to_data_x(points[:, 0]),
                                array_to_data_y(points[:, 1]),
                            )
                        ).T
                        ...
                        self.status_var.set(
                            f"Added {selection_type} selection to mask ({num_added} pixels)"
                        )

                    except Exception as e:
                        messagebox.showerror(
                            "Error",
                            f"Failed to process {selection_type} selection: {str(e)}",
                        )
                        traceback.print_exc()
                        self.status_var.set(
                            f"Error processing {selection_type} selection"
                        )
                        return
                else:
                    messagebox.showerror(
                        "Error", f"Unknown selection type: {selection_type}"
                    )
                    return
            else:
    ...
    def _save_mask(self):
        ...
        try:
            self.status_var.set(f"Saving mask to {Path(file_path).name}...")
            ...
            self.status_var.set(f"Mask saved to {Path(file_path).name}")
        except Exception as e:
    ...
    def _load_mask(self):
        ...
        try:
            self.status_var.set(f"Loading mask from {Path(file_path).name}...")
            # Load the mask
    ...
    def _use_workspace_chromatogram(self):
        ...
        try:
            ...
            if not chromatograms:
                messagebox.showinfo(
                    "Info", "No GCxGC_FID objects found in the current workspace"
                )
                return
            ...
            messagebox.showerror(
                "Error", f"Failed to use workspace chromatogram: {str(e)}"
            )
            import traceback
    ...
    def _apply_shift(self):
        ...
        try:
            ...
            if shift == 0:
                ...
                shifted_chrom_2d = shift_phase(
                    self.original_chromatogram.chrom_2D, shift
                )
